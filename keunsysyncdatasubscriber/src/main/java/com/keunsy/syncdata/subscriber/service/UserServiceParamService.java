/**    
 * 文件名：UserServiceParamService.java    
 *    
 * 版本信息：    
 * 日期：2015-9-29    
 ...
 */
@Service
public class UserServiceParamService extends BasicService {

    private final String TABLE_CLUSTER = " user_service_param ";
    ...
    //字段基本值(不含更新时间)
    private final String COLUMN_CLUSTER = "sn,user_id,param_key,param_value";
     ...
     * @return boolean 
     */
    public int uptToDataSource(UserServiceParam userServiceParam, String key_post) {
        ...
        if (server_array[0].contains("cluster")) {
            //进行更新
            result = updateDatabase(TABLE_CLUSTER, COLUMN_CLUSTER, WHERE_COLUMN_CLUSTER, userServiceParam);
        } else if (server_array[0].contains("cmpp")) {
            //进行数据转化，装填（集群无需转化，其他需要）
            GateUser gateUser = transformBean(userServiceParam, "cmpp");
            //进行更新
            ...
            //进行数据转化，装填（集群无需转化，其他需要）
            GateUser gateUser = transformBean(userServiceParam, "sgip");
            //进行更新
            ...
            //进行数据转化，装填（集群无需转化，其他需要）
            GateUser gateUser = transformBean(userServiceParam, "smgp");
            //进行更新
     ...
     * @return GateUser 
     */
    private GateUser transformBean(UserServiceParam userServiceParam, String server_key) {
        GateUser gateUser = new GateUser();

        gateUser.setUser_id(userServiceParam.getUser_id());
        /**
         * ?
         */
