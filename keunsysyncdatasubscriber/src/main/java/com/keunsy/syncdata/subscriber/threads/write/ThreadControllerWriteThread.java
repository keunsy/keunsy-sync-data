...
import com.keunsy.syncdata.subscriber.service.ThreadControllerService;
 ...
 * 类名称：ThreadControllerWriteThread    
 * 类描述：    
 * 创建人：chenrong1    
 ...
 */
@Service
public class ThreadControllerWriteThread extends Thread implements Stoppable {
    ...
    public ThreadControllerWriteThread() {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                // 队列获取数据
                ThreadController threadController = basicService.getInfoFromQueue(QueueCenter
                        .getThreadControllerQueue());
                if (threadController != null) {
                    int result = threadControllerService.uptToDataSource(threadController,
