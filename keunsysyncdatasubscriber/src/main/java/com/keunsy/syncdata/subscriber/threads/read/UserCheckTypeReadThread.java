...
@Service
public class UserCheckTypeReadThread extends Thread implements Stoppable {
    ...
    public UserCheckTypeReadThread() {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                //读取数据
                List<UserCheckType> list = basicService.readInfoList(RedisCfgCenter.KEY_POST_USERCHECKTYPE);
                //添加到队列
                basicService.addToQueue(list, QueueCenter.getUserCheckTypeQueue(),
                        RedisCfgCenter.KEY_POST_USERCHECKTYPE);
                //间隔时间
                long intervalTime = basicService.getIntervalTime(RedisCfgCenter.KEY_POST_USERCHECKTYPE);
