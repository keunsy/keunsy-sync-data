 ...
 */
@Service
public class ThreadControllerService extends BasicService {

    private final String TABLE_CLUSTER = " thread_controller ";
    ...
    //字段基本值(不含更新时间)
    private final String COLUMN_CLUSTER = "sn,server_ip,thread_name,action,status,thread_param,"
            + "thread_type,group_id,app_name";
     ...
     * @return boolean 
     */
    public int uptToDataSource(ThreadController threadController, String key_post) {
        ...
        if (server_array[0].contains("cluster")) {
            //进行更新
            result = updateDatabase(TABLE_CLUSTER, COLUMN_CLUSTER, WHERE_COLUMN_CLUSTER, threadController);
        } else if (server_array[0].contains("cmpp")) {
            //进行数据转化，装填（集群无需转化，其他需要）
            GateUser gateUser = transformBean(threadController, "cmpp");
            //进行更新
            ...
            //进行数据转化，装填（集群无需转化，其他需要）
            GateUser gateUser = transformBean(threadController, "sgip");
            //进行更新
            ...
            //进行数据转化，装填（集群无需转化，其他需要）
            GateUser gateUser = transformBean(threadController, "smgp");
            //进行更新
     ...
     * @return GateUser 
     */
    private GateUser transformBean(ThreadController threadController, String server_key) {
