...
import com.keunsy.syncdata.subscriber.service.UserServiceParamService;
 ...
 * 类名称：UserServiceParamWriteThread    
 * 类描述：    
 * 创建人：chenrong1    
 ...
 */
@Service
public class UserServiceParamWriteThread extends Thread implements Stoppable {

    @Resource
    private BasicService basicService;
    @Resource
    private UserServiceParamService userServiceParamService;

    private final Log log = CommonLogFactory.getLog("infoLog");

    private boolean isRunnable = false;

    public UserServiceParamWriteThread() {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                // 队列获取数据
                UserServiceParam userServiceParam = basicService.getInfoFromQueue(QueueCenter
                        .getUserServiceParamQueue());
                if (userServiceParam != null) {
                    int result = userServiceParamService.uptToDataSource(userServiceParam,
                            RedisCfgCenter.KEY_POST_USERSERVICEPARAM);
                    if (result < 1) {
