...
@Service
public class UserServiceInfoReadThread extends Thread implements Stoppable {
    ...
    public UserServiceInfoReadThread() {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                //读取数据
                List<UserServiceInfo> list = basicService.readInfoList(RedisCfgCenter.KEY_POST_USERSERVICEINFO);
                //添加到队列
                basicService.addToQueue(list, QueueCenter.getUserServiceInfoQueue(),
                        RedisCfgCenter.KEY_POST_USERSERVICEINFO);
                //间隔时间
                long intervalTime = basicService.getIntervalTime(RedisCfgCenter.KEY_POST_USERSERVICEINFO);
