...
@Service
public class UserServiceParamReadThread extends Thread implements Stoppable {
    ...
    public UserServiceParamReadThread() {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                //读取数据
                List<UserServiceParam> list = basicService.readInfoList(RedisCfgCenter.KEY_POST_USERSERVICEPARAM);
                //添加到队列
                basicService.addToQueue(list, QueueCenter.getUserServiceParamQueue(),
                        RedisCfgCenter.KEY_POST_USERSERVICEPARAM);
                //间隔时间
                long intervalTime = basicService.getIntervalTime(RedisCfgCenter.KEY_POST_USERSERVICEPARAM);
