...
@Service
public class SignInfoReadThread extends Thread implements Stoppable {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                //读取数据
                List<SignInfo> list = basicService.readInfoList(RedisCfgCenter.KEY_POST_SIGNINFO);
                //添加到队列
                basicService.addToQueue(list, QueueCenter.getSignInfoQueue(), RedisCfgCenter.KEY_POST_SIGNINFO);
                //间隔时间
                long intervalTime = basicService.getIntervalTime(RedisCfgCenter.KEY_POST_SIGNINFO);
