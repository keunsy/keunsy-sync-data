...
@Service
public class UserCountryPhoneCodePriceReadThread extends Thread implements Stoppable {
    ...
    public UserCountryPhoneCodePriceReadThread() {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                //读取数据
                List<UserCountryPhoneCodePrice> list = basicService
                        .readInfoList(RedisCfgCenter.KEY_POST_USERCOUNTRYPHONECODEPRICE);
                //添加到队列
                basicService.addToQueue(list, QueueCenter.getUserCountryPhoneCodePriceQueue(),
                        RedisCfgCenter.KEY_POST_USERCOUNTRYPHONECODEPRICE);
                //间隔时间
                long intervalTime = basicService.getIntervalTime(RedisCfgCenter.KEY_POST_USERCOUNTRYPHONECODEPRICE);
