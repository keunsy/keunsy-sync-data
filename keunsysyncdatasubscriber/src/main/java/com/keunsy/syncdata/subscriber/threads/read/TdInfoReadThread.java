...
@Service
public class TdInfoReadThread extends Thread implements Stoppable {
    ...
    public void run() {
        ...
        while (isRunnable) {
            try {
                //读取数据
                List<TdInfo> list = basicService.readInfoList(RedisCfgCenter.KEY_POST_TDINFO);
                //添加到队列
                basicService.addToQueue(list, QueueCenter.getTdInfoQueue(), RedisCfgCenter.KEY_POST_TDINFO);
                //间隔时间
                long intervalTime = basicService.getIntervalTime(RedisCfgCenter.KEY_POST_TDINFO);
